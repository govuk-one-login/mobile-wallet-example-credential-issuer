...
describe("issueDocumentSigningCertificate handler", () => {
  beforeEach(() => {
    ...
    jest.mocked(createCertificateRequestFromEs256KmsKey).mockResolvedValue("CSR" as unknown as Pkcs10CertificateRequest)
  ...
  describe("When processing starts", () => {
    ...
    it('issues a certificate', async () => {
      // ACT
      await lambdaHandlerConstructor(dependencies)(
        requestEvent,
        context,
      )
      ...
      expect(putObject).toHaveBeenCalledWith(
        'bucket',
        'keyId' + '/certificate.pem',
        'CERTIFICATE',
      )
      expect(putObject).toHaveBeenCalledWith(
        'bucket',
        'keyId' + '/certificate-metadata.json',
        '\"CERTIFICATE_DECODED\"',
      )
      ...
      expect(retrieveIssuedCertificate).toHaveBeenCalledWith(
        "CERT_ARN",
        "VALUE",
      )
    })
    ...
    it.each([
      ...
      // ACT
      const promise = lambdaHandlerConstructor(dependencies)(
        requestEvent,
        context,
      )
