...
public class DocumentFactory {
  ...
  private static final String MOBILE_DRIVING_LICENCE_DOCUMENT_TYPE = MOBILE_DRIVING_LICENCE_NAMESPACE + "mDL";
  ...
  private Map<String, List<IssuerSignedItem>> buildNameSpaces(final DrivingLicenceDocument drivingLicence) {
    try {
      ...
      for (Field field : drivingLicence.getClass().getDeclaredFields()) {
        ...
        IssuerSignedItem issuerSignedItem =
                issuerSignedItemFactory.build(
                        asSnakeCase, fieldValue);
        issuerSignedItems.add(issuerSignedItem);
      }

      return Map.of(MOBILE_DRIVING_LICENCE_NAMESPACE, issuerSignedItems);
    } catch (IllegalAccessException exception) {
      throw new MdlEncodingException(
              "Error occurred when accessing Driving Licence to build Name", exception);
    }
  }
