...
```bash
private-ca$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
private-ca$ sam local start-api
private-ca$ curl http://localhost:3000/
```
...
```bash
private-ca$ sam logs -n HelloWorldFunction --stack-name private-ca --tail
```
...
```bash
sam delete --stack-name private-ca
```
